...
Introduced a new feature in the game called SWORDS, which are displayed randomly in the arena. The player needs to dodge them by flipping over to the bottom of the stick by pressing the down arrow key. If the player touches the sword, they get killed and lose the game.

## Design Patterns Used:
1. **Singleton Design Pattern**: Ensures only one player object is present throughout the game.
2. **State Design Pattern**: Implements scene changes between first, second, and third scenes, i.e., Home Page, Game Page, and Game Over Page.

## JUnit Testing:
We have included 2 JUnit tests for some classes, available under the 'test' section in the code.

## Player Class

The Player class is a crucial component of the Stick Hero Game project, designed using the Singleton design pattern to ensure the existence of only one player object throughout the game.
...
The StickBridge class is a significant component of the Stick Hero Game project, representing the stick used to bridge gaps in the game. This class extends the functionality of the Rectangle class from JavaFX and incorporates features for stick manipulation during gameplay.
...
The Cherry class is an integral part of the Stick Hero Game project, extending the JavaFX ImageView class. It represents cherry objects within the game, serving as a visual element. Each instance of this class is associated with an image, providing the graphical representation of cherries on the screen.
...
The Cherry class allows for the easy integration of cherry objects into the Stick Hero Game, providing a visually appealing aspect to the gameplay.

## Swords Class

The Swords class is a component of the Stick Hero Game project, designed to extend the JavaFX ImageView class. It serves as a representation of sword objects within the game. Each instance of the class is associated with an image, providing a visual element for the swords displayed on the screen. The class includes a position attribute, allowing for dynamic tracking of the sword's location.
...
### Player Movement:
- The player can rotate the character by 180 degrees using the down arrow key and can rotate them back upwards by pressing the UP ARROW key.
...
- Updates the high score in a text file (`HighScore.txt`) and updates the total cherries after end of every game and before starting a new game.
